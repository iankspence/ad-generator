...
export class OutscraperService {
    ...
    async getGoogleMapsReviews(
        ...
        try {
            ...
            await this.pollTaskResults(userId, clinicId, response.data.id);
        } catch (error) {
            throw new Error(`Error fetching reviews: ${error.message}`);
        }
    }

    async pollTaskResults(userId: string, clinicId: string, taskId: string) {
        ...
        const checkResults = async () => {
            ...
            if (response.data.status === 'Pending') {
                ...
                response.data.data[0].reviews_data.forEach((review) => {
                    ...
                    this.reviewModelService.createGoogleReview(userId, clinicId, review);
                });
            }
        };
